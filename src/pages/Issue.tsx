...
import { issueItems } from "../utils/issues";
import q from "../assets/icon/q.png";
import back from "../assets/icon/back.png";
import Modal from "../components/Modal";
import { moreWallets, wallets } from "../utils/wallets";
...
const Issue = () => {
  ...
  return (
    <div>
      <div className="container mx-auto mt-20 md:pt-20 text-black font-rubik">
        {formTab && (
          <TabModal
            isOpen={formTab}
            ...
            <div className="grid grid-cols-4 gap-2 overflow-auto scrollbar-none max-h-96">
              {filteredWallets.map((wallet) => (
                ...
                >
                  ...
                  />
                  <div className="text-white mt-2 w-full text-center text-xs truncate">
                    {wallet.name}
                  </div>
                </div>
              ))}
        ...
        <div className="font-rubik text-2xl md:text-[40px] font-semibold uppercase text-center py-10">
          Issues
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4 lg:mx-auto container  md:px-32">
          {issueItems.map((item, index) => (
            <div
              className="cursor-pointer bg-pry flex text-center flex-col items-center p-4 border rounded hover:border-purple"
              key={index + 1}
